...
@section('title', 'Edit Role | Vogue Technics')
@section('content')
    <section>
        <div class="card">
            <h5 class="card-header">Edit Role</h5>
            <div class="card-body">
                <form class="ajax-form" action="{{ route('roles.update',$role->id) }}" method="POST" data-redirect="{{ url('roles') }}" class="g-3">
                    @csrf
                    @method('PUT')
                    <div class="row mb-3">
                        <div class="col-12">
                            <label class="form-label" for="name">Name</label>
                            <input type="text" id="name" name="name" class="form-control" placeholder="Manager" value="{{ $role->name }}" required/>
                        </div>
                    </div>
                    <div class="row">
